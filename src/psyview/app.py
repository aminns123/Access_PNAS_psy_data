...
class PsyView(App):
    ...
    BINDINGS += [('a', 'analysis', 'Analysis'), ('escape', 'hierarchy_focus', 'Hierarchy')]
    ...
    def __init__(self, adapter, export_dir=None):
        ...
        self.analysis_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix='psyview-analysis')
    ...
    async def on_mount(self):
        self.screen.set_class(self.size.height < 40, 'compact')
        self.set_interval(.5, self.check_figures)
        await self.redraw()

    def on_resize(self, event):
        self.screen.set_class(event.size.height < 40, 'compact')
    ...
    def check_figures(self):
        ...
        for process in self.figure_processes[:]:
            ...
            if code is not None:
                ...
                if code:
                    self.notify(f'Matplotlib failed (exit {code}). Check GUI backend/Tk installation. Full traceback: {LOG_PATH}', severity='error', timeout=15)
    ...
    async def redraw(self):
        ...
        if self.analysis.mode == 'interactive':
            adapter, level, filters, analysis = self.adapter, self.selection.active, self.selection.filters(), self.analysis
            self.spec = None
            self.query_one(PlotextPlot).plt.clear_figure()
            self.query_one(PlotextPlot).refresh()
            self.query_one('#info', Static).update(f'Computing interactive final {analysis.n_reversals} reversals… Navigation remains available.')
            async def compute():
                try:
                    spec = await asyncio.get_running_loop().run_in_executor(self.analysis_executor, adapter.get_plot, level, filters, analysis)
                    if generation == self.plot_generation:
                        self.display_spec(spec)
                except asyncio.CancelledError:
                    pass
                except Exception as exc:
                    logger.exception('Interactive analysis failed')
                    if generation == self.plot_generation:
                        self.query_one('#info', Static).update(f'Interactive analysis unavailable: {exc}')
            self.analysis_task = asyncio.create_task(compute())
            return
        try:
            self.display_spec(self.adapter.get_plot(self.selection.active, self.selection.filters()))
        except Exception as exc:
    ...
    def display_spec(self, spec):
        self.spec = spec
        TerminalPlotRenderer().render(self.query_one(PlotextPlot), spec)
        info = ' | '.join(f'{k}: {v}' for k, v in spec.metadata.items())
        self.query_one('#info', Static).update(f'Current level: {self.adapter.levels()[self.selection.active].name}\n{info}\n{spec.notes}')

    def show_analysis(self):
        focus = ' [FOCUSED: ENTER switches mode; ←/→ change N; A/ESC returns]' if self.analysis_focus else ' [A: focus controls]'
        mode = 'ARCHIVED PNAS' if self.analysis.mode == 'archived' else f'INTERACTIVE — median of final {self.analysis.n_reversals} reversals'
        self.query_one('#analysis', Static).update('Analysis: '+mode+focus)

    def action_analysis(self):
        if not hasattr(self.adapter, 'interactive'):
            self.notify('Interactive analysis is not available for this adapter.')
            return
    ...
    async def change_n(self, delta=0, edge=None):
        if self.analysis.mode != 'interactive':
            self.notify('Press Enter to explicitly switch to interactive analysis.')
            return
        maximum = self.adapter.interactive().max_n()
        n = 1 if edge == 'first' else maximum if edge == 'last' else min(maximum, max(1, self.analysis.n_reversals+delta))
        self.analysis = AnalysisState('interactive', n)
        await self.redraw()
    ...
    async def action_first(self):
        if self.analysis_focus:
            await self.change_n(edge='first')
            return
    ...
    async def action_enter(self):
        if self.analysis_focus:
            mode = 'interactive' if self.analysis.mode == 'archived' else 'archived'
            self.analysis = AnalysisState(mode, self.analysis.n_reversals)
            await self.redraw()
            return
    ...
    def action_matplotlib(self):
        if self.spec:
            ...
            try:
                self.figure_processes.append(MatplotlibPlotRenderer().open(self.spec))
                self.notify('Starting Matplotlib window')
            except Exception as exc:
                logger.exception('Cannot open Matplotlib')
                self.notify(str(exc), severity='error')
        else:
            self.notify('No prepared plot is available for this selection.', severity='warning')
    ...
    async def action_reload(self):
        try:
            ...
            logger.exception('Cannot reload dataset')
