...
def json_scalar(value):
    if isinstance(value, np.generic):
        return value.item()
    raise TypeError(
        f'Unsupported plot payload value: {type(value).__name__}'
    )
...
class MatplotlibPlotRenderer:
    def figure(self, spec, figure=None):
        ...
        for series in spec.series:
            display_color = _series_color(series)
        ...
        for axis in ('x', 'y'):
            ...
            getattr(ax, 'set_' + axis + 'scale')(
                scale
            )
            getattr(ax, 'set_' + axis + 'lim')(
                limits
            )
...
def show_payload(path, close_after=None):
    ...
    from matplotlib.backends.registry import (
        backend_registry,
    )

    _, gui = backend_registry.resolve_backend(
        matplotlib.get_backend()
    )

    if gui is None:
        raise RuntimeError(
            f'Matplotlib backend {matplotlib.get_backend()} cannot open windows. '
            'Install Python with Tk support or configure a usable GUI backend '
            'via MPLBACKEND.'
        )
    ...
    if close_after is not None:
        timer = fig.canvas.new_timer(
            interval=close_after
        )
        timer.single_shot = True
        timer.add_callback(
            lambda: plt.close(fig)
        )
        timer.start()
