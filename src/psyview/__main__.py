...
def _adapter_from_config(config):
    adapter_name = config['dataset'].get(
        'adapter',
        'csv',
    )
    if adapter_name == 'pnas':
        from .data.pnas_psychophysics import PNASAdapter
        return PNASAdapter(config)
    if adapter_name == 'lateral':
        from .data.lateral_sensitivity import LateralAdapter
        return LateralAdapter(config)
    if adapter_name == 'csv':
        return CSVAdapter(config)
    raise DatasetError(
        f'Unsupported configured adapter: {adapter_name!r}'
    )


def main():
    parser = argparse.ArgumentParser(
        description='Read-only terminal experimental data explorer'
    )
    parser.add_argument('--data-root', type=Path)
    parser.add_argument(
        '--debug',
        action='store_true',
        help='Write verbose diagnostics to the temporary psyview.log file',
    )
    parser.add_argument('--config', type=Path)
    parser.add_argument(
        '--export-dir',
        type=Path,
        default=Path.home() / 'psyview-exports',
    )
    parser.add_argument(
        '--detect',
        action='store_true',
        help='Report tentative CSV structure without choosing scientific axes',
    )
    parser.add_argument(
        '--save-config',
        type=Path,
        help='Save detected YAML for editing; refuses to overwrite or write inside dataset',
    )
    ...
    try:
        if args.data_root is None:
            if args.detect or args.config or args.save_config:
                raise DatasetError(
                    '--detect/--config/--save-config require --data-root.'
                )
        ...
        if args.detect:
            ...
            if args.save_config:
                ...
                with destination.open(
                    ...
                    file.write(output)
                print(
                    f'Saved tentative configuration: {destination}'
                )
