...
def validate_config(config):
    ...
    if dataset.get('adapter', 'csv') not in ('csv', 'pnas'):
        raise DatasetError('Unknown dataset adapter; supported adapters are csv and pnas.')
    ...
    for level in hierarchy:
        if not isinstance(level, dict) or not all(isinstance(level.get(key), str) for key in ('name', 'column', 'source')):
            raise DatasetError('Each hierarchy entry needs string name, column and source fields.')
        if set(level) - {'name', 'column', 'source', 'units'}:
            raise DatasetError(f'Unsupported hierarchy fields: {set(level) - {"name", "column", "source", "units"}}')
