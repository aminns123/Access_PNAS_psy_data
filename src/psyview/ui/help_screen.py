...
class HelpScreen(ModalScreen):
    ...
    def compose(self):
        yield Static(
            ...
            'View overrides are session-only and are scoped to the current\\n'
            'hierarchy level/view type. Manual bounds may intentionally crop data.\\n'
            'Log scale is refused when the empirical row contains non-positive values.\\n'
            'Categorical X axes disable numeric X controls.\\n\\n'
            'Hidden compatibility shortcuts: X/Y still toggle axis scale directly;\\n'
            'Home/End still select first/last values. They are omitted from the footer.\\n\\n'
